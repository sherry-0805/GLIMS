# use: R CMD BATCH --args luad_preprocessing.R
...
node<-read.csv('./data/network/luad/max_connected_graph_nodes.csv')
...
# expression
expr<-fread("./expr/TCGA-LUAD.htseq_fpkm.tsv.gz", h=T, check.names=F)
...
nrow(expr1)
expr_can<-expr1
write.csv(expr1,'./expr/tcga_luad_expr.csv',row.names=FALSE)
# get normal data
...
gene<-expr$gene
expr1<-data.frame(gene,expr1)
write.csv(expr1,'./expr/tcga_luad_expr_normal.csv',row.names=FALSE)
expr_nor<-expr1
...
rate<-c(1:nrow(FC))
FC<-cbind(FC,rate)
FC$rate<-(FC$mean_cancer+0.0000001)/(FC$mean_nor+0.0000001)
log_v<-c(1:nrow(FC))
FC<-cbind(FC,log_v)
...
write.csv(FC,"./expr/tcga_luad_expr_FC_detail.csv", row.names = FALSE)
FC<-FC[,c(1,6)]
colnames(FC)[2]<-"FC"
write.csv(FC,"./expr/tcga_luad_expr_FC.csv", row.names = FALSE)


# mutation
mut<-fread("./mut/TCGA-LUAD.muse_snv.tsv.gz",h=T, check.names=F)
data<-data.frame(mut)
...
mut[is.na(mut)]<-0
mut<-mut[,-c(1)]
write.csv(mut,"./mut/luad_mut.csv",row.names=FALSE)
...
write.csv(freq_data,'./mut/tcga_luad_freq.csv',row.names=FALSE)

# copy number
cn<-fread('./cn/TCGA-LUAD.gistic.tsv.gz',h=T, check.names=F)
...
cn2<-cn2[,-c(1)]
cn<-cn2
write.csv(cn,'./cn/luad_cn.csv',row.names=FALSE)


# methylation
methy<-fread('./methy/TCGA-LUAD.methylation450.tsv.gz',h=T, check.names=F)
colnames(methy)[1]<-'cg'
cg<-methy$cg
...
write.csv(methy,"./methy/luad_methy.csv",row.names=FALSE)
