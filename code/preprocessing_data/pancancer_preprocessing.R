# use: R CMD BATCH --args pan_preprocessing.R
...
node<-read.csv('./data/network/pancancer/max_connected_graph_nodes.csv')
...
setwd('./data/DATA_TCGA/pancancer')
# load_sample
...
# expression
expr<-fread("./expr/GDC-PANCAN.htseq_fpkm-uq.tsv.gz", h=T, check.names=F)
...
nrow(expr1)
expr_can<-expr1
write.csv(expr1,'./expr/tcga_pancancer_expr.csv',row.names=FALSE)
# get normal data
...
gene<-expr$gene
expr1<-data.frame(gene,expr1)
write.csv(expr1,'./expr/tcga_pancancer_expr_normal.csv',row.names=FALSE)
expr_nor<-expr1
...
write.csv(FC,"./expr/tcga_pancancer_expr_FC_detail.csv", row.names = FALSE)
FC<-FC[,c(1,6)]
colnames(FC)[2]<-"FC"
write.csv(FC,"./expr/tcga_pancancer_expr_FC.csv", row.names = FALSE)

# mutation
mut<-fread("./mut/GDC-PANCAN.muse_snv.tsv.gz",h=T, check.names=F)
data<-data.frame(mut)
...
mut[is.na(mut)]<-0
mut<-mut[,-c(1)]
write.csv(mut,"./mut/pancancer_mut.csv",row.names=FALSE)
...
write.csv(freq_data,'./mut/tcga_pancancer_freq.csv',row.names=FALSE)

# copy number
cn<-fread('./cn/GDC-PANCAN.gistic.tsv.gz',h=T, check.names=F)
...
cn2<-cn2[,-c(1)]
cn<-cn2
write.csv(cn,'./cn/pancancer_cn.csv',row.names=FALSE)


# methylation
methy<-fread('./methy/GDC-PANCAN.methylation450.tsv.gz',h=T, check.names=F)
colnames(methy)[1]<-'cg'
cg<-methy$cg
...
write.csv(methy,"./methy/pancancer_methy.csv",row.names=FALSE)
