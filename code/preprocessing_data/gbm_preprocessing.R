...
node<-read.csv('./data/network/gbm/max_connected_graph_nodes.csv')
...
# expression
expr<-fread("./expr/TCGA-GBM.htseq_fpkm.tsv.gz", h=T, check.names=F)
...
nrow(expr1)
expr_can<-expr1
write.csv(expr1,'./expr/tcga_gbm_expr.csv',row.names=FALSE)
# get normal data
...
gene<-expr$gene
expr1<-data.frame(gene,expr1)
write.csv(expr1,'./expr/tcga_gbm_expr_normal.csv',row.names=FALSE)
expr_nor<-expr1
...
write.csv(FC,"./expr/tcga_gbm_expr_FC_detail.csv", row.names = FALSE)
FC<-FC[,c(1,6)]
colnames(FC)[2]<-"FC"
write.csv(FC,"./expr/tcga_gbm_expr_FC.csv", row.names = FALSE)

# mutation
mut<-fread("./mut/TCGA-GBM.muse_snv.tsv.gz",h=T, check.names=F)
data<-data.frame(mut)
...
mut[is.na(mut)]<-0
mut<-mut[,-c(1)]
write.csv(mut,"./mut/gbm_mut.csv",row.names=FALSE)
# freq
freq<-c(1:nrow(mut))
mut1<-cbind(freq,mut)
count<-c(1:nrow(mut1))
...
write.csv(freq_data,'./mut/tcga_gbm_freq.csv',row.names=FALSE)




# copy number
cn<-fread('./cn/TCGA-GBM.gistic.tsv.gz',h=T, check.names=F)
...
cn2<-cn2[,-c(1)]
cn<-cn2
write.csv(cn,'./cn/gbm_cn.csv',row.names=FALSE)

# methylation
methy<-fread('./methy/TCGA-GBM.methylation27.tsv.gz',h=T, check.names=F)
colnames(methy)[1]<-'cg'
cg<-methy$cg
methy1<-methy[,colnames(methy) %in% can_sample$sample,with=FALSE]
methy2<-data.frame(cg,methy1)
probe<-fread('./methy/illuminaMethyl27_hg38_GDC',h=T, check.names=F)
colnames(probe)[1]<-'id'
methy_get <-methy2 %>%
...
write.csv(methy,"./methy/gbm_methy.csv",row.names=FALSE)
